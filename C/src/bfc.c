...
static ByteCode *code; /* the code generated by the compiler */
...
/**
 * Ensures that there's enough space in the 'code' (bytecode array) for
...
ByteCode *compile(const char *filepath)
{
    ...
    pos = 0;
    size = INIT_CODE_SIZE;
    code = (ByteCode*)emalloc(sizeof(ByteCode) * size);
...
static void ensure_space(void)
{
    ...
    size = size + (size / 2);
    code = (ByteCode*)erealloc(code, sizeof(ByteCode) * size);
}

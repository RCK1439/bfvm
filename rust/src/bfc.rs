        ...
        while self.token != Token::EndOfFile {
            match self.token {
                ...
                _ => {
                    ...
                    return Err(BFVMError::from_position(pos, format!("unexpected token: {:?}", self.token), BFVMErrSeverity::Error))
                }
            }
        }
            ...
            match self.token {
                ...
                Token::BracketRight => {
                    if let Some(open) = braces.pop() {
                        ...
                        let pos: SourcePosition = self.lexer.current_position();
                        return Err(BFVMError::from_position(pos, String::from("stack empty"), BFVMErrSeverity::Error));
                    }
                },
                Token::EndOfFile => {
                    if let Some(open) = braces.pop() {
                        let pos: SourcePosition = open.1;
                        return Err(BFVMError::from_position(pos, String::from("no matching ']'"), BFVMErrSeverity::Error));
                    } else {
                        let pos: SourcePosition = self.lexer.current_position();
                        return Err(BFVMError::from_position(pos, String::from("stack empty"), BFVMErrSeverity::Error));
                    }
